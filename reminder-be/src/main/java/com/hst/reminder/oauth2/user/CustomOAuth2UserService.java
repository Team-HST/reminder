...
public class CustomOAuth2UserService extends DefaultOAuth2UserService {
	...
	public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
		...
		OAuth2Provider oAuth2Provider = OAuth2Provider.get(registrationId);
		...
		Member member = processOAuth2User(oAuth2Provider, userInfo);
		return memberRepository.save(member);
	}

	// OAuth2 가입 여부 확인 & 가입 처리
	private Member processOAuth2User(OAuth2Provider oAuth2Provider, OAuth2UserInfo userInfo) {
		Member member;
		Optional<Member> memberOpt = memberRepository.findByEmail(userInfo.getEmail());
		if (memberOpt.isPresent()) {
			member = memberOpt.get();
			if (member.getSsoProvider() != oAuth2Provider) {
			...
			member = Member.createMemberBySosial(userInfo, oAuth2Provider);
		}
		return member;
	}
